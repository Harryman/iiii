...
io.setup("P8_20", io.IN)
#io.cleanup()
io.add_event_detect("P8_20",io.RISING)
last = datetime.datetime.now()
flow = 0

def mlPerSec2lPerMin(val):
	return (val*60)/1000
...
try:
	while True:
		if((datetime.datetime.now() - datetime.timedelta(seconds=1))> last): 
			a0 = ain2c(adc.read('AIN0'))
			a1 = ain2c(adc.read('AIN1'))
			a2 = ain2c(adc.read('AIN2'))
			a3 = ain2c(adc.read('AIN3'))
			os.system('clear')
			...
			print "Flow ml/s",
			print flow
			f.write(time.ctime()+","+str(a0)+","+str(a1)+","+str(a2)+","+str(a3)+","+str(flow)+"\n")
			flow = 0
			last = datetime.datetime.now()
	if io.event_detected("P8_20"):
		flow += 91.667 
